  ...
  useEffect(() => {
    ...
    const fetchInfo = async () => {
      ...
      const newPortal = await Promise.all(portalData.map(async (item) => ({
        ...item,
        decimal: await Promise.all(item.rewardsToken.map(async (addr) => getDecimal(addr))),
      ...
      const updatedAccordionData = depositeData.map((item) => ({
        tokenAddress: newPortal.filter((subitem) => isAddress(item.portal) === isAddress(subitem.portalAddress))[0].rewardsToken,
        ...
        endDate: newPortal.filter((subitem) => isAddress(item.portal) === isAddress(subitem.portalAddress))[0].endDate,
        amount: item.amount.map((amount, key) => formatUnits(amount, newPortal.filter((subitem) => isAddress(item.portal) === isAddress(subitem.portalAddress))[0].decimal[key]).toString()),
        label:
