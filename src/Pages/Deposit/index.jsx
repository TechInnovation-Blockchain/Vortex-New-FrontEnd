...
import {
  DepositConfirm, ElevatedBtn, GlassCard, PopupDropdown, InputDatePicker, InputText, InputText2,
} from 'components'
...
const LOADING_DATA_TITLES = {
  [TransactionStatus.TRANSACTION_PENDING]: 'PORTAL BEING CREATED',
  [TransactionStatus.TRANSACTION_SUCCESS]: 'PORTAL SUCCESSFULLY CREATED',
  [TransactionStatus.TRANSACTION_FAILED]: 'PORTAL CREATION FAILED',
  [TransactionStatus.TRANSACTION_REJECTED]: 'PORTAL CREATION REJECTED',
}

const STAKE_LOADING_TITLE = {
  [TransactionStatus.TRANSACTION_PENDING]: 'PORTAL DEPOSIT PENDING',
  [TransactionStatus.TRANSACTION_SUCCESS]: 'PORTAL DEPOSIT SUCCESS',
  [TransactionStatus.TRANSACTION_FAILED]: 'PORTAL DEPOSIT FAILED',
  [TransactionStatus.TRANSACTION_REJECTED]: 'PORTAL DEPOSIT REJECTED',
}

const defaultPortalDate = new Date()
defaultPortalDate.setMonth(defaultPortalDate.getMonth() + 1)
defaultPortalDate.setDate(defaultPortalDate.getDate() + 1)
...
const Deposit = ({ size }) => {
  ...
  const [selectedPortalOptionDeposit, setSelectedPortalOptionDeposit] = useState('')
  const [selectedTokenOptionDeposit, setSelectedTokenOptionDeposit] = useState('')
  const [selectedTokenOptionCreate, setSelectedTokenOptionCreate] = useState('')
  const [tokenContractAddresses, setTokenContractAddresses] = useState()
  const [endDate, setEndDate] = useState(defaultPortalDate)
  const [totalCount, setTotalCount] = useState(0)

  useEffect(() => {
    if (!loadingData) return
    if (loadingData.data === 'transaction success!') {
  ...
  const [tokenOptions, setTokenOptions] = useState([ // eslint-disable-line @typescript-eslint/no-unused-vars
    {
      label: 'XIO',
  ...
  const [requestedTokenOptions, setRequestedTokenOptions] = useState([]) // eslint-disable-line @typescript-eslint/no-unused-vars
  ...
  const paginationArr = (arr) => arr?.reduce((curr, item, index) => {
      ...
      <div style={{ textAlign: 'left' }}>
        ...
        <div>
          <p>
            ...
            Reward Token:
            {' '}
            {rewardsTokenNames.map((item, index) => (
          ...
          <p>
            <b>Note:</b>
            {' '}
            There will be several transactions to confirm in your
  ...
  useEffect(() => {
    ...
    const fetchEndDate = async () => {
      ...
      const dayDifference = (((newEndDate - block) * 365) / 2102400) * 1000 * 3600 * 24
  ...
  const addToken = () => { // eslint-disable-line consistent-return
    if (
      !(
        minRewardRatio
        && minRewardRatio > 0
  ...
  useEffect(() => {
    ...
    if (tokenArr.length === 1) {
      setPortalName(`Portal ${parseInt(totalCount, 10) + 1} ${tokenArr[0].token_name}`)
      return
    }
      ...
      Content: () => (
        <DepositConfirm
          ...
          confirmed={async () => {
            ...
            const stakingToken = tokenOptions.filter((val) => val.label === selectedTokenOptionCreate)[0].address
  ...
  const tokenError = (tokenResponse.startsWith('invalid') || tokenResponse.startsWith('empty')) && tokenResponse
  ...
  const handleQuantityChange = (index, percentage, changed = false) => {
    const temp = [...quantity]
    if (changed) temp[index] = parseFloat(percentage) > 0 ? parseFloat(percentage) : ''
    else {
    ...
    <div className="content p-4" style={{ maxWidth: '600px' }}>
      <main>
        ...
        <div className="modal__search-section">
          ...
          />
          <button className="modal__search-btn" onClick={() => handleSearch()} type="button">
            Search
          </button>
        </div>
        <section style={{}}>
  ...
  const handleSearch = () => {
    ...
    const filteredData = portalOptionsNew?.filter((item) => (
  ...
  const handleSearch2 = () => {
    if (!searchValue) return
    const filteredData = tokenOptions?.filter((item) => (
      item.label.toLowerCase().includes(searchValue.toLowerCase())
  ...
  return (
    <GlassCard width="450px" height="auto">
                              ...
                              sublabel={(
                                <span className="d-flex align-items-center gap-1">
                                  <span
                                    ...
                                    onClick={() => handleQuantityChange(index, 25)}
                                    role="button"
                                    tabIndex={0}
                                    onKeyDown={() => { }}
                                  >
                                  ...
                                  <span
                                    ...
                                    onClick={() => handleQuantityChange(index, 100)}
                                    role="button"
                                    tabIndex={0}
                                    onKeyDown={() => { }}
                                  >
                    ...
                    >
                      <div>
                        ...
                        <div>
                          ...
                          >
                            {selectedTokenOptionCreate || 'Select a token'}
                          </button>
                        </div>
                      </div>
                    </div>
                        ...
                        <div className="">
                          ...
                          {tokenArr.map((token) => (
                            <p className="text-center p-0 m-0" key={token.token_name}>
                              <DeleteIcon
                                onClick={() => setTokenArr([
                                  ...tokenArr.filter((t) => token.token_name !== t.token_name),
                                ])}
                                color="secondary"
                      ...
                      <div className="row">
                        <div className="col-10">
                          <div className="row">
                            ...
                            >
                              {tokenResponse.length > 0 && !tokenError && (
                                <p
                                  style={{
                                    color: 'white',
                                    fontSize: '13px',
                            ...
                            <div className="col-6">
                              <InputText
                                ...
                                onChange={(v) => setMinRewardRatio(parseInt(v, 10))}
      ...
      <ThemeModal show={modal.show} onClose={modal.onClose}>
      ...
      {loadingData.data && typeof loadingData.data === 'string' && (
        <ThemeResponse title={LOADING_DATA_TITLES[loadingData.data]} type={loadingData.data} />
      )}
      {stakeLoading.data && (
        <ThemeResponse title={STAKE_LOADING_TITLE[stakeLoading.data]} type={stakeLoading.data} />
      )}
    </GlassCard>
  )
}
