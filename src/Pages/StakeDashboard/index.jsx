...
import { unStake } from "../../contracts/functions/portalFunctions";
...
const StakeDashboard = () => {
  ...
  const fetchInfo = async () => {
    ...
    const updatedAccordionData = stakeData.map((item) => ({
      address: newPortal.filter(
        (subitem) => isAddress(item.portal) === isAddress(subitem.portalAddress)
      )[0].stakingToken,
      ...
      unstaked_time:
        unstakeData.filter((t) => t.portal === item.portal).length !== 0
          ? formatDate(
              unstakeData.filter((t) => t.portal === item.portal)[0].createdAt
            )
          : 0,
    }));
      ...
      Content: () => (
        <DepositConfirm
          ...
          getDescription={() => (
            <div>
              <p>
                You are unstaking <b>{accordionData[active].token}</b> from{" "}
                <b>{accordionData[active].label}</b>. You will no longer earn
                rewards from that portal. Please claim any available rewards
                earned to date.
              </p>
            </div>
          )}
          ...
          confirmed={async () => {
            await unStake(accordionData[active].portal, account, () => {
              modal.onClose();
            });
            modal.onClose();
          }}
  ...
  return (
    <>
      ...
      {unstaking.data === "transaction pending!" && (
        <ThemeResponse title="PORTAL UNSTAKE PENDING" type="pending" />
      )}
      {unstaking.data === "transaction success!" && (
        <ThemeResponse title="PORTAL UNSTAKE SUCCESS" type="success" />
      )}
      {unstaking.data === "transaction failed!" && (
        <ThemeResponse title="PORTAL UNSTAKE FAILED" type="failed" />
      )}
      {unstaking.data === "transaction rejected!" && (
        <ThemeResponse title="PORTAL UNSTAKE REJECTED" type="rejected" />
      )}
    </>
  );
};
